...
describe("journey progress", () => {
  ...
  it("spaces seven dots and paints only the finished story card", () => {
    ...
    expect(css).toMatch(/#yours\.card--lavender[\s\S]*?#fff1a0/);
    expect(css).toMatch(/#beeffe/);
    expect(css).toMatch(/#dfefff/);
    expect(css).toMatch(/#8eeaf2/);
    expect(css).toMatch(/#ffdee5/);
    expect(css).toMatch(/#eee1fa/);
    expect(css).toMatch(/#d0f0fd/);
    expect(css).toMatch(/rgba\(255, 241, 160, 0\) 60%/);
    expect(css).toMatch(/rgba\(142, 234, 242, 0\) 60%/);
    expect(css).not.toMatch(/#fedfd7|#a5fad1|#ffe6b5/);
    const plain = css.match(/\.card--lavender \{[^}]+\}/)?.[0] ?? "";
    expect(plain).toMatch(/#ebe7fb/);
    expect(plain).not.toMatch(/#fff1a0/);
