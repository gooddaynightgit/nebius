...
| API | `/api/captures` ingest (`source=app` enforces photo-day rules), `/api/yours` open tonight's story (same-day replace weaves a fresh one), `/api/email` gate (landing path), `/api/weave` story, `/api/story` + `/api/story/audio` playback. |
...
5. **1:50** After YOURS, **Keep** saves the photo with the story. A kinder photo still fits today — replace, save, and YOURS writes again. After midnight the link expires.
