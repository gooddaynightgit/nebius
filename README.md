...
| GET/POST | `/api/payfast/checkout` | Health, or an auto-submitting Payfast form. Charge is 16.00 ZAR; a confirmed payment credits 25 moments |
...
The pack is **25** good moments for **R16 ZAR / $0.88**. Payfast is charged the string **16.00**. `item_name` is `GoodDayNight — 25 good moments`. An ITN `amount_gross` of at least **16.00** grants 25. A gross below 16.00 grants 0 and is not stored, unless that `pf_payment_id` is already on a row — then the handler returns 200 and leaves `game` unchanged.
...
- `SANDBOX` defaults to false (`www.payfast.co.za`). Leave it unset or set `SANDBOX=false` with the **live** `PF_MERCHANT_ID`, `PF_MERCHANT_KEY`, and `PF_PASSPHRASE`. Vercel Preview uses those live keys and `SANDBOX=false`. Set `SANDBOX=true` only for `sandbox.payfast.co.za`, and only with sandbox keys. Do not mix them.
...
A confirmed ITN adds 25 to `game` on the DynamoDB item in `goodfans` (override with `FANS_TABLE`). The partition key `order` is the normalized buyer email, so “Already bought?” and a photo upload can find the same row. Checkout stores that email in `custom_str1`. The ITN credits `custom_str1` when it is an email, including when Payfast’s `email_address` is a different account. A non-email `custom_str1` (the vault id from an older checkout) falls back to `email_address`. A second confirmed pack for that email adds another 25 onto whatever `game` is left. It does not replace that balance. The Payfast `pf_payment_id` is stored in `paymentIds` and a duplicate ITN returns 200 without adding again, including a replay of an older 5.00 charge whose id is already stored. `emailVaultId` (`em_` plus the email hash) is metadata only, for the Blob/S3 vault. Balances are not written to `sleepcoachfans` or to the old entitlement JSON. A moment save is one new day’s photo that can become My good moment; that upload decrements `game` by 1 only while it is still above 0. On `/app`, **Already bought?** checks that balance and opens Take / Upload while the pack is active. Replay, Share, joy picks, and saving again the same day do not spend a moment. An underpaid ITN (`amount_gross` below `16.00`) credits 0 and is not stored.
...
3. Open `/moments`. The price block says **Unlock 25 good moments weaved for** **R16** **ZAR / $0.88**. Payfast charges **16.00 ZAR**. `GET /api/payfast/checkout` should show `"sandbox": false`, `"amount": "16.00"`, `"moments": 25`, and no secrets.
4. The browser returns to `/app?paid=1`. After the ITN, choose **Already bought?** and enter the same email. Take / Upload should enable, with 25 moment saves added onto the existing balance. Cancel returns to `/moments?cancelled=1`.
