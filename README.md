...
- ITN uses Payfast’s notify signature, which is not the checkout string. Every posted field is signed in the order received, including blanks (`name_last=&custom_str2=`), and the walk stops at `signature`. Values are PHP `urlencode` (spaces as `+`, uppercase hex, no trim), then the passphrase, then lowercase MD5. A blank-skipping digest is accepted only as a fallback. The handler still requires our `merchant_id`, `payment_status` `COMPLETE`, and `amount_gross` of at least `5.00`, POSTs the raw notify body to Payfast’s validate URL, and requires the response body to start with `VALID`. A rejection is logged as `[payfast-itn] rejected` plus a short reason. The log does not include the passphrase, the signature, or the buyer email.
- Any failure before the `goodfans` credit returns **500** so Payfast retries. **200** is returned only after that update. The same `pf_payment_id` does not credit a second pack.
